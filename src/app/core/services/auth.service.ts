...
export class AuthService {
  ...
  private loadUser(): any {
    ...
    applyDoctorProfileImageAliases(parsed);
    return parsed;
  }
  ...
  registerDoctor(data: any): Observable<any> {
    const body: any = { ...data };
    const raw = body.image_profile;
    const trimmed = typeof raw === 'string' ? raw.trim() : raw;
    if (trimmed) {
      body.image_profile = trimmed;
      body.imageProfile = trimmed;
    } else {
      delete body.image_profile;
    }
    return this.http.post<any>(`${this.api}/auth/doctor/register`, body);
  }
  ...
  loginDoctor(email: string, password: string): Observable<any> {
    return this.http.post<any>(`${this.api}/auth/doctor/login`, { email, password }).pipe(
      tap((r: any) => {
        if (r.success && r.data.token) {
          const doctor = { ...r.data.doctor, role: 'doctor' };
          applyDoctorProfileImageAliases(doctor);
          this.save(r.data.token, doctor);
        }
      })
    );
  }
